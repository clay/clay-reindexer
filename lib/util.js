...
function streamFetchJson() {
  return streamFetch.apply(this, arguments)
    .tap(res => {
      if (res.status === 404) throw new errors.request404(JSON.stringify(arguments));
    })
...
function getSiteMemoized() {
   ...
   * @return {Stream}
   */
  return function (prefix, elasticPrefix) {
    let url, host, path, index;
    ...
    return streamSearch({
      ...
      .tap(result => cache[prefix] = result);
  };
}
