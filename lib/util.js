...
function streamPageUris(path, fetchOpts) {
  const url = `${AMPHORA_HOST}${path === '/' ? '' : path}/pages`;
...
function getPageUrl(uri, version) {
  ...
  let url = `${AMPHORA_HOST}${path === '/' ? '' : path}/pages/${clayUtils.getPageInstance(uri)}`;
...
function streamAllPageUris() {
  console.log('stream all page uris');
  return getSites()
