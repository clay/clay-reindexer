 ...
 * @return {Stream} doc partial
 */
function applyHandlers(doc, opts) {
  const composedUrl = util.getPageUrl(doc.uri, opts.prefix) + '.json',
    handlerOpts = Object.assign({}, opts);

  return getDocSite(doc, opts.elasticPrefix)
    .flatMap(site => {
      Object.assign(handlerOpts, {site});
      return util.streamFetchJson(composedUrl);
    })
    .flatMap(util.streamComponents)
    .flatMap(cmpt => applyCmptHandler(cmpt, handlerOpts))
 ...
 * @param  {Object} opts Handler opts, passed to each handler as third argument.
 * @return {Stream} of doc partials merged into doc
 */
function applyCmptHandler(cmpt, opts) {
  const componentName = clayUtils.getComponentName(cmpt._ref),
    handler = opts.handlers[componentName];
  let result;

  if (!handler) return h.of({});
  result = handler(cmpt._ref, _.omit(cmpt, '_ref'), opts);
