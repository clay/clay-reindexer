  ...
  describe(filename, function () {
    ...
    it ('converts pages to docs and puts docs to Elastic', function () {
      const mockPrefix = 'http://foo.com',
        ...
        mockSite = {slug: 'foo'},
        mockContext = Object.assign({}, mockOpts, {site: mockSite});

      util.getSite
        .withArgs(mockOpts.prefix, mockOpts.elasticPrefix)
        .returns(h.of({slug: 'foo'}));
      util.streamPageUris
        .withArgs(mockPrefix)
        .returns(h([mockPageUri1, mockPageUri2]));
      reindexUtil.pageToDoc
        .withArgs('foo.com/pages/1', sinon.match(mockContext))
        .returns(h.of(mockDoc1));
      reindexUtil.pageToDoc
        .withArgs('foo.com/pages/2', sinon.match(mockContext))
