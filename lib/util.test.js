...
describe(_.startCase(filename), function () {
  ...
  describe('streamFetchJson', function () {
    ...
    it ('automatically parses body for json', function () {
      nodeFetchStub.resolves({ok: true, json: () => Promise.resolve({foo: 'bar'})});
    ...
    it ('throws a 404 error if status is 404', function (done) {
      nodeFetchStub.resolves({ok: false, status: 404, text: () => Promise.resolve()});
      fn(mockUrl, mockOpts)
        .toPromise(Promise)
        .then((results) => done('expected error but got ' + JSON.stringify(results)))
        .catch(err => {
          expect(err).to.be.an.instanceof(errors.request404);
          done();
        });
    });
  });
  ...
  describe('streamFetchText', function () {
    ...
    it ('automatically parses body for text', function () {
      nodeFetchStub.resolves({ok: true, text: () => Promise.resolve('foo')});
    ...
    it ('throws a 404 error if status if 404', function (done) {
      nodeFetchStub.resolves({status: 404, ok: false, text: () => Promise.resolve()});
      fn(mockUrl, mockOpts)
        .toPromise(Promise)
        .then(result => done('expected error but got ' + JSON.stringify(result)))
        .catch((err) => {
          expect(err).to.be.an.instanceof(errors.request404);
          done();
        });
  ...
  describe('streamPageUris', function () {
    ...
    it ('sends correct request and streams each uri', function () {
      nodeFetchStub.withArgs('http://foo.com/pages')
        .resolves({ok: true, json: () => Promise.resolve(mockUris)});
  ...
  describe('logResult', function () {
    ...
    it ('if result is error, logs error stack', function () {
      const mockResults = [
        {status: 'error', error: {stack: 'foo'}}
      ];

      h(mockResults)
        .tap(fn())
        .done(() => {
          const result = JSON.parse(console.log.getCall(0).args[0]);

          sandbox.restore();
          expect(result.error).to.equal('foo');
        });
    });
  });
  ...
  describe('getSite', function () {
    ...
    it ('ignores trailing slash in prefix', function () {
      mockEsClient.search.resolves({hits: {hits: [{_source: {foo: 'bar'}}]}});

      return fn(mockPrefix5)
        .toPromise(Promise)
        .then(() => {
          const esOpts = mockEsClient.search.getCall(0).args[0];

          expect(esOpts.body.query.bool).to.eql(
            {filter: [{term: {host: 'foo.com'}}, {term: {path: ''}}]}
          );
        });
    });
  });
